...
def verarbeite_partie(zuege):
    """
    Verarbeitet die Liste der Züge einer einzelnen Partie.

    Args:
        zuege (list): Liste der Züge im UCI-Format.
    """
    board = chess.Board()
    for uci_str in zuege:
        try:
            ...
            if move in board.legal_moves:
                ...
                # Hier kannst du deine Logik zur Verarbeitung der Koordinaten einfügen
...
if __name__ == "__main__":
    ...
    partien_inhalte = pgnread2.lese_pgn_datei_als_string(pgn_datei)

    for inhalt in partien_inhalte:
        zuege = pgnread2.extrahiere_partie_zuege(inhalt)
        if zuege:
            verarbeite_partie(zuege)
        else:
